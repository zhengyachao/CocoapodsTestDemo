...
{
    // 发起微信支付
    [[YKSDKManager shareManager] lunchWechatPayWithOrderNum:self.orderNumber orderCreateTime:self.orderCreateTime];
}
...
- (void)_onApplePayButton:(UIButton *)button
{
    ...
    {
        [self requestProductData:@"marketjinglihuiyuan"];
    }else
    {
        NSLog(@"不允许程序内付费");
    }
...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response
{
    ...
    for (SKProduct *pro in product)
    {
        ...
        if ([pro.productIdentifier isEqualToString:@"marketjinglihuiyuan"])
        {
            p = pro;
        }
    }
...
- (void)completeTransaction:(SKPaymentTransaction *)transaction
{
    ...
    NSString *voucher = [[NSString alloc] initWithData:[GTMBase64 encodeData:data] encoding:NSUTF8StringEncoding];
    NSLog(@"%@",voucher);
    [[SKPaymentQueue defaultQueue] finishTransaction:transaction];
}
